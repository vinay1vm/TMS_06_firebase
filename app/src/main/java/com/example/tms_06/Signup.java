        ...
        submit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if(names.isEmpty() || emails.isEmpty() || doornos.isEmpty() || phnos.isEmpty() || Rpasswords.isEmpty()){
                    ...
                    databaseReference.child("users").child(phnos).child("full_name").setValue(names);
                    databaseReference.child("users").child(phnos).child("email").setValue(emails);
                    databaseReference.child("users").child(phnos).child("door_no").setValue(doornos);
                    databaseReference.child("users").child(phnos).child("ph_no").setValue(phnos);
                    databaseReference.child("users").child(phnos).child("password").setValue(Rpasswords);
